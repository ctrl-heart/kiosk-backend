...
const getTotalEventsCount = async () => {
  ...
  const updateEventById = async (event_id, eventData) => {
    ...
    const query = `UPDATE events SET ${fields.join(', ')} WHERE event_id = $${index} RETURNING *`;
...
const deleteEvent = async (event_id) => {
  const result = await database.query('DELETE FROM Events WHERE event_id = $1 RETURNING *', [event_id]);
  return result.rows[0];
};
...
const createEvent = async (event) => {
  const { title, description, location, time, capacity, price, category_id } = event;
  const result = await database.query(
...
module.exports = { getAllEvents, getEventById, getTotalEventsCount,updateEventById,deleteEvent,getEventAttendees,createEvent };