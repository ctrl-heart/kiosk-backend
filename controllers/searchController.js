...
exports.searchEvents = async (req, res) => {
    ...
    try {
        ...
        if (search) {
            conditions.push('(e.title ILIKE $' + (values.length + 1) + ' OR e.description ILIKE $' + (values.length + 1) + ' OR e.location ILIKE $' + (values.length + 1) + ')');
            values.push(`%${search}%`);
        }
...
exports.filterEvents = async (req, res) => {
    const { category_name, start_date, end_date, price_min, price_max } = req.query;
    ...
    try {
        ...
        if (start_date && end_date) {
            conditions.push('DATE(e.time) BETWEEN $' + (values.length + 1) + ' AND $' + (values.length + 2));
        ...
        if (price_min && price_max) {
            conditions.push('e.price BETWEEN $' + (values.length + 1) + ' AND $' + (values.length + 2));
