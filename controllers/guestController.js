...
const guestBookingController = {
    bookEventForGuest: async (req, res) => {
      ...
      try {
        ...
        const existingBooking = await BookingModel.findBookingByUserAndEvent(user.user_id, event_id);
        if (existingBooking) {
        ...
        doc.fontSize(12).text(`Booking Date: ${bookingDate.toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}`);
        doc.text(`Booking Time: ${bookingDate.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}`);
        doc.moveDown();
        ...
        const formattedEventDate = `${eventDate.toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })} ${eventDate.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}`;
